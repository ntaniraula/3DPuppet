...
{  public static void main(String[] args)throws IOException
   {  
         new SphTriaObj("sphere.dat", "spherefinal.dat");
    }
}

class SphTriaObj
{  Vector v = new Vector(); // Vertices
   Vector t = new Vector(); // Triangular faces
   int nV, codeRadix;
   ...
   {  Input inp = new Input(inputFile);
    ...
    {  if (p<q)
       {  ht.put(new Integer(codeRadix*p+q),new Integer(++nV));
          Point3D P = (Point3D)v.elementAt(p),
                  Q = (Point3D)v.elementAt(q);
    ...
    {  int key = p < q ? (codeRadix*p+q): (codeRadix *q+p);
       Integer iObj = (Integer)ht.get(new Integer(key));
